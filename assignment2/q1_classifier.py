...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        ### YOUR CODE HERE
        config = self.config
        self.input_placeholder = tf.placeholder(dtype=tf.float32, shape=(config.batch_size, config.n_features))
        self.labels_placeholder = tf.placeholder(dtype=tf.int32, shape=(config.batch_size, config.n_classes))
        ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        W = tf.Variable(tf.random_uniform(shape=[self.config.n_features, self.config.n_classes]))
        b = tf.Variable(tf.random_uniform(shape=[self.config.n_classes]))
